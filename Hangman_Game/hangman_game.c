...
void getMovieName(){
    ...
    srand(time(0));
    random_line = rand() % count + 1; // generates a random number

    count = 0;
    file = fopen("movies.txt", "r");

    /*Add the random movie to "movie_name[]" array */
    while (fgets(line, sizeof(line), file)) {
        count++;
...
/*check if guessed character is already used or not */
...
/* check if guessed character is allowed */
...
/* check if guessed character is present in movie name */
